...
- (void) signRequest:(NSMutableURLRequest *)request
{
	...
	NSString *schemeHost 	= [NSString stringWithFormat:@"%@://%@", [url scheme], [url host]];
	NSString *uri 			= [[url absoluteString] substringFromIndex:schemeHost.length];
	
	//	NSLog(@"uri: %@", uri);
	
	NSString *iso8601Date 			= [self iso8601Date];
	NSString *canonicalizedResource = [NSString stringWithFormat:@"%@:%@", uri, iso8601Date];
	NSString *signature 			= [self hmacSha256:canonicalizedResource salt:[SWGConfiguration sharedConfig].apiSecret];
	//	NSLog(@"canonicalizedResource: %@", canonicalizedResource);
	//	NSLog(@"signature: %@", signature);
	
	[[SWGConfiguration sharedConfig] setApiKey:iso8601Date 								forApiKeyIdentifier:@"X-Boomtown-Date"];
	[[SWGConfiguration sharedConfig] setApiKey:[SWGConfiguration sharedConfig].apiToken forApiKeyIdentifier:@"X-Boomtown-Token"];
	[[SWGConfiguration sharedConfig] setApiKey:signature 								forApiKeyIdentifier:@"X-Boomtown-Signature"];
	
	[request setValue:iso8601Date 								forHTTPHeaderField:@"X-Boomtown-Date"];
	[request setValue:[SWGConfiguration sharedConfig].apiToken	forHTTPHeaderField:@"X-Boomtown-Token"];
